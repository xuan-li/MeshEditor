...
void OTEViewer::LoadMesh()
{
	...
	for (auto viter = mesh_.vertices_begin(); viter != mesh_.vertices_end(); ++viter) {
		...
		Vec2d uv = mesh_.texcoord2D(v);
		if (!(-1000 < uv[0] && uv[0] < 1000))
			mesh_.set_texcoord2D(v, Vec2d(0, 0));
	}
...
void OTEViewer::SaveCurrentScene()
{
	...
	SurfaceMesh new_mesh = mesh;

	for (auto viter = new_mesh.vertices_begin(); viter != new_mesh.vertices_end(); ++viter) {
		VertexHandle v = *viter;
		Vec3d p = new_mesh.point(v);
		Vec3d n = mesh.normal(v);
		Eigen::Vector4f p_h(p[0], p[1], p[2], 1.);
		Eigen::Vector4f current_p = world * p_h;
		new_mesh.set_point(v, Vec3d(current_p[0], current_p[1], current_p[2]));
	}
	std::string fname = igl::file_dialog_save();
	if (fname.length() == 0)
		return;
	OpenMesh::IO::Options opt;
	opt += OpenMesh::IO::Options::VertexTexCoord;
	OpenMesh::IO::write_mesh(new_mesh, fname, opt);
}
